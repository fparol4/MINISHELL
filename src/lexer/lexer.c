...
/*   Updated: 2026/05/09 21:02:19 by g-alves-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_rules *rules)
{
	...
	while (!scanner_is_end(input))
	{
		...
		if (props & rules->is_operator)
			get_operator(manager, input, rules);
		else if (props & (L_WORD | P_DQUOTE | P_SQUOTE | P_SYMBOL))
		...
		t_rules *rules)
{
	...
	while (!scanner_is_end(input))
	{
		...
		if ((state == P_NONE) && (props & (P_SPACE | rules->is_operator)))
			break ;
		scanner_advance(input);
	}
