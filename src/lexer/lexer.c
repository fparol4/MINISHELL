/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   lexer.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: g-alves- <g-alves-@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/04 22:09:36 by g-alves-          #+#    #+#             */
/*   Updated: 2026/03/09 23:37:53 by g-alves-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char			*ft_get_token(t_manager *manager, t_list_token *token,
						char *input);
static char			*ft_capture_word(char *input);
static char			*get_redir_or_pipe(char *input);
static t_token_type	define_type(char *type);

t_manager	*lexer_controll(char	*input)
{
	t_manager		manager;
	t_list_token	*token;

	token = add_token_to_list(&manager, NULL, NULL);
	ft_get_token(&manager, token, input);
}

static char	*ft_get_token(t_manager *manager, t_list_token *token, char *input)
{
	char	*word;
	char	*type;

	if (!input)
		return ;
	while (input)
	{
		ft_jump_space(input);
		word = ft_capture_word(input);
		if (!word)
			return ;
		token = add_token_to_list(manager, word, TOKEN_WORD);
		ft_jump_space(input);
		type = get_redir_or_pipe(input);
		if (!type)
			return ;
		token = add_token_to_list(manager, type, define_type(type));
	}

}

static char	*ft_capture_word(char *input)
{
	char	*word;
	char	*first_position;

	if (!input)
		return ;
	if (*input == '"' || *input == '\'')
		write(1, "Caso ainda não tratado", 24);
	first_position = input;
	while (*input != ' ' && *input != '|' && *input != '<' && *input != '>')
		*(input)++;
	word = malloc((input - first_position) * sizeof(char) + 1);
	if (!word)
		return ;
	ft_memcpy(word, first_position, (input - first_position));
	word[input - first_position] = '\0';
	return (word);
}

static char	*get_redir_or_pipe(char *input)
{
	char		*type;
	char		*first_position;
	size_t		len;

	if (!input)
		return ;
	first_position = input;
	len = 0;
	if ((*input == '<' && *(input + 1) == '<')
		|| (*input == '>' && *(input + 1) == '>'))
		len = 2;
	if (*input == ' ' || *input == '|' || *input == '<' || *input == '>')
		len = 1;
	type = malloc(len * sizeof(char) + 1);
	if (!type)
		return ;
	ft_memcpy(type, first_position, len);
	type[len] = '\0';
	*(input)++;
	return (type);
}

static t_token_type	define_type(char *type)
{
	if (ft_strncmp(type, "|", 1) == 0)
		return (TOKEN_PIPE);
	if (ft_strncmp(type, "<", 1) == 0)
		return (TOKEN_REDIR_IN);
	if (ft_strncmp(type, ">", 1) == 0)
		return (TOKEN_REDIR_OUT);
	if (ft_strncmp(type, ">>", 2) == 0)
		return (TOKEN_APPEND);
	if (ft_strncmp(type, "<<", 2) == 0)
		return (TOKEN_HEREDOC);
	return (NULL);
}
