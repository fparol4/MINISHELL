...
describe(rn_redir)
{
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_in_XXXXXX";
		char			*raw[] = {"cat", NULL};
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_out_XXXXXX";
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_app_XXXXXX";
		char			*raw[] = {"echo", "new", NULL};
		t_env			*env = NULL;
		t_command		node;
		t_parser_redir	redir;
	...
	it("fails when a redirection target expands to many args")
	{
		...
		rn_test_attach_redirs(&node, &redir, 1);
		asserteq(rn_execute(&node, &env, STDIN_FILENO, NULL), 1);
	...
	{
		char			dir[] = "tester/_tmp/minishell_redir_space_XXXXXX";
	...
	{
		char			dir[] = "tester/_tmp/minishell_redir_denied_XXXXXX";
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_missing_input";
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_override_in_XXXXXX";
		char			*raw[] = {"cat", NULL};
		t_env			*env = env_init((char *[]){"PATH=/usr/bin:/bin", NULL});
		t_command		node;
		t_parser_redir	redirs[2];
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_override_heredoc_XXXXXX";
		char			*raw[] = {"cat", NULL};
		t_env			*env = env_init((char *[]){"PATH=/usr/bin:/bin", NULL});
		t_command		node;
		t_parser_redir	redirs[2];
	...
	{
		char			path[] = "tester/_tmp/minishell_redir_only_XXXXXX";
		t_env			*env = NULL;
		t_command		node;
		t_parser_redir	redir;
		char			*out;
